...
print('=== SUCCESSFUL SQLITE TO POSTGRESQL MIGRATION ===')
...
renk_count = 0
kod_letters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'
for i, item in enumerate(items):
    if '|' in item:
        ...
        if len(parts) >= 2:
            ad = parts[1].strip()
            if ad:
                kod = kod_letters[i % len(kod_letters)]  # Tek karakter
...
print(f'   Total Records Migrated: {3 + renk_count + beden_count + gider_count + kasa_count}')
print('\\n ALL SQLITE DATABASE MIGRATED TO POSTGRESQL!')
print(' PRODUCTION SYSTEM READY!')
print(' LOGIN: https://nuviabutik.com/kullanici/login/')
print('  Username: admin | Password: nuviaadmin')
