...
for user_row in sqlite_users:
    print(f'User: {user_row[ username]} - {user_row[email]} - Role: {user_row[role] if role in user_row.keys() else N/A}')
...
with transaction.atomic():
    for user_row in sqlite_users:
        try:
            # Role field kontrolü
            role_value = user_row[role] if role in user_row.keys() else calisan
...
# Test authentication - nuviaadmin şifresi ile test
