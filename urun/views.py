...
def urun_ekle(request):
    ...
    if request.method == 'POST':
        try:
            ...
            alis_fiyati = Decimal(alis_fiyati_str) if alis_fiyati_str else Decimal('0')
            kar_orani = Decimal(kar_orani_str) if kar_orani_str else Decimal('50')
            satis_fiyati = Decimal(satis_fiyati_str) if satis_fiyati_str else Decimal('0')
        ...
        try:
            ...
            if ad:
                ...
                if logo:
                    marka.logo = logo
                marka.save()
                messages.success(request, f'{ad} markası başarıyla güncellendi!')
                return redirect('urun:marka_listesi')
            else:
                messages.error(request, 'Marka adı boş olamaz!')
        except Exception as e:
...
def urun_duzenle(request, urun_id):
    ...
    if request.method == 'POST':
        try:
            ...
            urun.alis_fiyati = Decimal(alis_fiyati_str) if alis_fiyati_str else Decimal('0')
            urun.kar_orani = Decimal(kar_orani_str) if kar_orani_str else Decimal('50')
            urun.satis_fiyati = Decimal(satis_fiyati_str) if satis_fiyati_str else Decimal('0')
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    messages.success(request, f'✅ {varyant.varyasyon_adi} için {miktar} adet stok giriş işlemi başarıyla tamamlandı!')
                    return redirect('urun:stok_giris')
                    
            except Exception as e:
                messages.error(request, f'❌ Stok giriş işlemi sırasında hata: {str(e)}')
    else:
        form = StokGirisForm()
            ...
            try:
                with transaction.atomic():
                    ...
                    if varyant.stok_miktari < miktar:
                        messages.error(request, f'❌ Yetersiz stok! {varyant.varyasyon_adi} için mevcut stok: {varyant.stok_miktari} adet')
                    ...
                    messages.success(request, f'✅ {varyant.varyasyon_adi} için {miktar} adet stok çıkış işlemi başarıyla tamamlandı!')
                    return redirect('urun:stok_cikis')
                    
            except Exception as e:
                messages.error(request, f'❌ Stok çıkış işlemi sırasında hata: {str(e)}')
    else:
        form = StokCikisForm()
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    if fark == 0:
                        messages.warning(request, f'⚠️ {varyant.varyasyon_adi} için stok miktarı değişmedi.')
                    ...
                    messages.success(request, f'✅ {varyant.varyasyon_adi} için stok düzeltme işlemi başarıyla tamamlandı! (Eski: {eski_miktar}, Yeni: {yeni_miktar})')
                    return redirect('urun:stok_duzeltme')
                    
            except Exception as e:
                messages.error(request, f'❌ Stok düzeltme işlemi sırasında hata: {str(e)}')
    else:
    ...
    if request.method == 'POST':
        if 'arama' in request.POST:
            ...
            if arama_terimi:
                try:
                    # Önce barkod ile ara
                    varyant = UrunVaryanti.objects.select_related('urun').get(barkod=arama_terimi)
                    ...
                    if varyantlar.exists():
                        ...
                        messages.error(request, f'❌ "{arama_terimi}" için ürün bulunamadı!')
        
        elif 'islem' in request.POST and request.POST.get('varyant_id'):
            # Stok azaltma işlemi
            try:
                with transaction.atomic():
                    varyant = UrunVaryanti.objects.get(id=request.POST.get('varyant_id'))
                    azaltilacak_miktar = int(request.POST.get('miktar', 0))
                    aciklama = request.POST.get('aciklama', '').strip()
                    
                    if azaltilacak_miktar <= 0:
                        messages.error(request, '❌ Azaltılacak miktar 0\'dan büyük olmalıdır!')
                        return redirect('urun:sayim_eksigi')
                    
                    if azaltilacak_miktar > varyant.stok_miktari:
                        messages.error(request, f'❌ Mevcut stoktan ({varyant.stok_miktari} adet) fazla azaltılamaz!')
                        return redirect('urun:sayim_eksigi')
                    ...
                    messages.success(request, f'✅ {varyant.varyasyon_adi} için {azaltilacak_miktar} adet stok azaltıldı!')
                    return redirect('urun:sayim_eksigi')
            ...
            try:
                with transaction.atomic():
                    ...
                    if arttirilacak_miktar <= 0:
                        messages.error(request, '❌ Arttırılacak miktar 0\'dan büyük olmalıdır!')
                    ...
                    messages.success(request, f'✅ {varyant.varyasyon_adi} için {arttirilacak_miktar} adet stok eklendi!')
                    return redirect('urun:sayim_fazlasi')
            ...
            with transaction.atomic():
                if guncelleme_tipi == 'oran':
                    ...
                    if oran != 0:
                        for urun in urunler:
                            if oran > 0:  # Artış
                                urun.satis_fiyati = urun.satis_fiyati * (Decimal('1') + oran / Decimal('100'))
                            else:  # Azalış
                                urun.satis_fiyati = urun.satis_fiyati * (Decimal('1') + oran / Decimal('100'))
                            urun.save()
                            updated_count += 1
                    ...
                    yeni_kar_orani = Decimal(str(request.POST.get('kar_orani', 50)))
                    for urun in urunler:
                        if urun.alis_fiyati > 0:
                            urun.kar_orani = yeni_kar_orani
                            urun.satis_fiyati = urun.alis_fiyati * (Decimal('1') + yeni_kar_orani / Decimal('100'))
                            urun.save()
                            updated_count += 1
            
            messages.success(request, f'✅ {updated_count} ürünün fiyatı başarıyla güncellendi!')
            
        except Exception as e:
            messages.error(request, f'❌ Fiyat güncelleme sırasında hata: {str(e)}')
