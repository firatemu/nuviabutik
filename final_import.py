...
print('=== Final Import to PostgreSQL ===')

# Renkler import (newline düzelt)
...
with open('/tmp/renkler.txt', 'r', encoding='utf-8') as f:
    for line in f:
        ...
        if len(parts) >= 2:
            renk_id, ad = parts[0], parts[1]
            renk, created = Renk.objects.get_or_create(
                ad=ad,
                defaults={'id': int(renk_id)}
            )
            if created:
                renk_count += 1
...
with open('/tmp/bedenler.txt', 'r', encoding='utf-8') as f:
    for line in f:
        parts = line.strip().replace('\\n', '').split('|')
        if len(parts) >= 2:
            beden_id, ad = parts[0], parts[1]
            beden, created = Beden.objects.get_or_create(
                ad=ad,
                defaults={'id': int(beden_id)}
            )
            if created:
                beden_count += 1
...
with open('/tmp/kasalar_fixed.txt', 'r', encoding='utf-8') as f:
    for line in f:
        parts = line.strip().replace('\\n', '').split('|')
        if len(parts) >= 4:
            kasa_id, ad, tip, bakiye = parts[0], parts[1], parts[2], parts[3]
            try:
